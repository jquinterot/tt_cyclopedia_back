...
class ForumLike(Base):
    ...
    id = Column(String(255), primary_key=True, default=lambda: shortuuid.uuid())
    forum_id = Column(
        String(255), ForeignKey(get_fk_reference("forums"), ondelete="CASCADE"), nullable=False
    )
    user_id = Column(
...
class ForumComment(Base):
    ...
    comment_likes = relationship(
        "ForumCommentLike", backref="comment", cascade="all, delete-orphan"
    )
    if schema_kwargs:
        __table_args__ = schema_kwargs  # type: ignore
...
class ForumCommentLike(Base):
    ...
    comment_id = Column(
        String(255),
        ForeignKey(get_fk_reference("forum_comments"), ondelete="CASCADE"),
        nullable=False,
    )
    user_id = Column(
        String(255), ForeignKey(get_fk_reference("users"), ondelete="CASCADE"), nullable=False
