...
class TestForums:
    ...
    def test_create_forum_success(self, client, auth_headers):
        ...
        response = client.post("/forums", json={
            "title": f"Test Forum {unique}",
    ...
    def test_update_forum_success(self, client, auth_headers):
        unique = str(uuid.uuid4())
        create_resp = client.post("/forums", json={
            "title": f"Update Forum {unique}",
            "content": "Original content"
        }, headers=auth_headers)
        assert create_resp.status_code == status.HTTP_201_CREATED
        forum_id = create_resp.json()["id"]
        update_resp = client.put(f"/forums/{forum_id}", json={
            "title": "Updated Title",
    ...
    def test_update_forum_not_found(self, client, auth_headers):
        response = client.put("/forums/fake-forum-id", json={
            "title": "Updated"
        }, headers=auth_headers)
        assert response.status_code == status.HTTP_404_NOT_FOUND
    ...
    def test_forum_comments_success(self, client, auth_headers):
        unique = str(uuid.uuid4())
        create_resp = client.post("/forums", json={
            "title": f"Comment Forum {unique}",
            "content": "Forum with comments"
        }, headers=auth_headers)
        assert create_resp.status_code == status.HTTP_201_CREATED
        forum_id = create_resp.json()["id"]
        comment_resp = client.post(f"/forums/{forum_id}/comments", json={
            "comment": "Nice forum!"
        }, headers=auth_headers)
    ...
    def test_forum_comments_update_success(self, client, auth_headers):
        unique = str(uuid.uuid4())
        forum_resp = client.post("/forums", json={
            "title": f"Update Comment Forum {unique}",
            "content": "Content"
        }, headers=auth_headers)
        ...
        update_resp = client.put(f"/forums/{forum_id}/comments/{comment_id}", json={
            "comment": "Updated"
        }, headers=auth_headers)
        assert update_resp.status_code == status.HTTP_200_OK
    ...
    def test_forum_comments_delete_success(self, client, auth_headers):
        unique = str(uuid.uuid4())
        forum_resp = client.post("/forums", json={
            "title": f"Delete Comment Forum {unique}",
            "content": "Content"
        }, headers=auth_headers)
        assert forum_resp.status_code == status.HTTP_201_CREATED
        forum_id = forum_resp.json()["id"]
    ...
    def test_forum_comments_like_success(self, client, auth_headers):
        unique = str(uuid.uuid4())
        forum_resp = client.post("/forums", json={
            "title": f"Like Comment Forum {unique}",
            "content": "Content"
        }, headers=auth_headers)
        assert forum_resp.status_code == status.HTTP_201_CREATED
        forum_id = forum_resp.json()["id"]
        ...
        assert comment_resp.status_code == status.HTTP_201_CREATED
        comment_id = comment_resp.json()["id"]
        like_resp = client.post(f"/forums/{forum_id}/comments/{comment_id}/like", headers=auth_headers)
        assert like_resp.status_code == status.HTTP_200_OK
    ...
    def test_general_forum_comments_crud(self, client, auth_headers):
        unique = str(uuid.uuid4())
        forum_resp = client.post("/forums", json={
            "title": f"General Comment Forum {unique}",
            "content": "Content"
        }, headers=auth_headers)
        assert forum_resp.status_code == status.HTTP_201_CREATED
        forum_id = forum_resp.json()["id"]
        ...
        create_resp = client.post("/forums/comments", json={
        ...
        # Update
        update_resp = client.put(f"/forums/comments/{comment_id}", json={
            "comment": "Updated general"
        }, headers=auth_headers)
        assert update_resp.status_code == status.HTTP_200_OK
    ...
    def test_get_forum_comments_by_forum_id(self, client, auth_headers):
        unique = str(uuid.uuid4())
        forum_resp = client.post("/forums", json={
            "title": f"Forum ID Comments {unique}",
            "content": "Content"
        }, headers=auth_headers)
        assert forum_resp.status_code == status.HTTP_201_CREATED
        forum_id = forum_resp.json()["id"]
        client.post("/forums/comments", json={
            "comment": "By forum id",
            "forum_id": forum_id
        }, headers=auth_headers)
    ...
    def test_get_main_forum_comments_by_forum_id(self, client, auth_headers):
        unique = str(uuid.uuid4())
        forum_resp = client.post("/forums", json={
            "title": f"Main Forum Comments {unique}",
            "content": "Content"
        }, headers=auth_headers)
        assert forum_resp.status_code == status.HTTP_201_CREATED
        forum_id = forum_resp.json()["id"]
    ...
    def test_get_forum_comment_replies_by_forum_id(self, client, auth_headers):
        unique = str(uuid.uuid4())
        forum_resp = client.post("/forums", json={
            "title": f"Replies Forum {unique}",
            "content": "Content"
        }, headers=auth_headers)
        assert forum_resp.status_code == status.HTTP_201_CREATED
        forum_id = forum_resp.json()["id"]
        parent_resp = client.post("/forums/comments", json={
            "comment": "Parent",
            "forum_id": forum_id
        }, headers=auth_headers)
